...
module.exports = async (ctx) => {
    ...
    const disableEmbed = true || ctx.params.disableEmbed;
    ...
    const emojiApi = `https://mashiro.top/bilibili_emojis/index.json`;
    const emojiList = await ctx.cache.tryGet(emojiApi, async () => {
        const result = await got.get(emojiApi);
        return result.data;
    });
        ...
        item: data.map((item) => {
            ...
            if (item.display.emoji_info) {
                ...
                emoji.forEach((item) => {
                    ...
                    // );
                    const short_code = item.text.replace("[", "").replace("]", "");
                    if (short_code in emojiList) {
                        data_content = data_content.replace(
                        new RegExp(`\\${item.text}`, "g"),
                        ` :${emojiList[short_code]}: `
                        );
                    }
                });
            }
